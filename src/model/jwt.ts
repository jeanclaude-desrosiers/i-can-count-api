...
const RSA_PRIV_PATH = path.join('..', 'res', 'rsa.priv.pem');
const RSA_PUB_PATH = path.join('..', 'res', 'rsa.pub.pem');
...
export class JWT {
    ...
    public static from_user(user: User): Promise<string> {
        return fs.readFile(RSA_PRIV_PATH, 'utf-8')
            .then((rsa_priv_str: string) => importSPKI(rsa_priv_str, 'RS256'))
    ...
    public static to_user(db: DatabaseClient, jwt: string): Promise<User | undefined> {
        return fs.readFile(RSA_PUB_PATH, 'utf-8')
            .then((rsa_pub_str: string) => importPKCS8(rsa_pub_str, 'RS256'))
